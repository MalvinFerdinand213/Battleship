	...
	def create_button_board(self):
		...
		for i in range(n_row):
			...
			for j in range(n_column):
				button = tk.Button(self.frame_rows[i], bg="purple", image=self.init_img_btn,height=button_height, width=button_width, command=lambda x=i, y=j :self.game.button_clicked(x, y))
				row.append(button)
